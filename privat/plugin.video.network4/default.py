...
def view_root():
    ...
    add_dir('[COLOR gold]★ Sport[/COLOR]', build_url(action='list', path='sport'))
    ...
    add_dir('[COLOR grey]Details oldal mentése hibakereséshez[/COLOR]',
...
def view_list(path):
    items = n4.sport_list(path)
    if not items:
        notify('Nincs találat – az oldalt a hibakereső mappába mentettem', t=8000)
    for it in items:
...
def _ask_slug():
    kb = xbmc.Keyboard(ADDON.getSetting('last_slug') or '',
                       'network4.hu/sport/details/... link vagy slug')
    kb.doModal()
...
def view_dump():
    ...
    if not s:
        return
    page = s if s.startswith('http') else 'sport/details/' + s.strip('/').split('/')[-1]
    html = n4.get(page)
    slug = page.rstrip('/').split('/')[-1]
    p1 = n4.save_debug('details_%s.html' % slug, html)
