# -*- coding: utf-8 -*-
"""
Muteki Fansub (mutekifansub.hu) - privát, email+jelszó auto-login kliens.

Belépés:
    A felhasználó a beállításokban megadja az email címét és jelszavát.
    Az addon a /login űrlapon keresztül automatikusan bejelentkezik (a form
    _csrf_token mezőjét is kitölti), a kapott munkamenet-sütit (PHPSESSID)
...
Lejátszás:
    ...
    A data-file minőségenként tartalmazza a KÖZVETLEN mp4 URL-eket:
        [720p]https://.../..01 (720p) [HASH].mp4|[1080p]https://...|[2K]...|[4K]...
...
DEFAULT_BASE = 'https://mutekifansub.hu/'
        ...
        for c in json.loads(raw or '[]'):
            try:
                _SESSION.cookies.set(c['name'], c['value'],
                                     domain=c.get('domain') or 'mutekifansub.hu',
                                     path=c.get('path') or '/')
            except Exception:  # noqa
                pass
    except Exception as exc:  # noqa
    ...
    for m in re.finditer(r'<form\b([^>]*)>(.*?)</form>', html or '', re.DOTALL | re.I):
        ...
        for im in re.finditer(r'<input\b([^>]*)>', body, re.I):
            ...
            if itype == 'password' and not pwd:
                pwd = name
            elif itype in ('email', 'text') and name != '_csrf_token':
...
def login():
    ...
    if '_csrf_token' not in data:
        cm = re.search(r'name="_csrf_token"\s+value="([^"]+)"', page)
        if cm:
            data['_csrf_token'] = cm.group(1)
...
# ---------------------------------------------------------------------------
def latest_episodes(limit=60):
    """A főoldal értesítés-listájából a legfrissebb epizódok:
    [{'id', 'title', 'art'}]. Több-epizódos bejegyzést részenként bont."""
    html = cached_get('', min(_cache_minutes(), 10) * 60, referer=base_url())
    if not html:
        return []
    starts = [m.start() for m in
              re.finditer(r'<div class="notification-item[^"]*"\s+data-id="\d+"', html)]
    starts.append(len(html))
    out, seen = [], set()
    for i in range(len(starts) - 1):
        block = html[starts[i]:starts[i + 1]]
        if '/episode/watch/id/' not in block:
            continue
        tm = re.search(r'<div class="notification-item-title">.*?<font>(.*?)</font>',
                       block, re.DOTALL)
        title = _txt(tm.group(1)) if tm else 'rész'
        am = re.search(r'<img\s+src="([^"]+)"', block)
        art = _abs(am.group(1)) if am else None
        pairs = re.findall(
            r'<span class="episode-number">([^<]+)</span>.*?/episode/watch/id/(\d+)',
            block, re.DOTALL)
        if pairs:
            for epnum, eid in pairs:
                if eid in seen:
                    continue
                seen.add(eid)
                out.append({'id': eid, 'title': '%s – %s' % (title, _txt(epnum)),
                            'art': art})
        else:
            wm = re.search(r'/episode/watch/id/(\d+)', block)
            if wm and wm.group(1) not in seen:
                seen.add(wm.group(1))
                out.append({'id': wm.group(1), 'title': title, 'art': art})
        if len(out) >= limit:
            break
    log('%d legfrissebb rész' % len(out))
    return out[:limit]
...
def parse_projects(html=None):
    ...
    for idx, m in enumerate(matches):
        ...
        art = _abs(am.group(1)) if am else None
        epm = re.search(r'<span class="project-episodes">([^<]+)</span>', body)
        ...
        out.append({'slug': slug, 'title': title, 'status': status,
                    ...
                    'tags': _attr(rest, 'data-tags'), 'art': art,
                    'episodes': episodes, 'plot': plot, 'alt': alt.lower()})
    return out


def filters_meta(html=None):
    """A /projects oldal végén lévő JSON-ból: (seasons[(key,label)], genres[str])."""
    html = html if html is not None else projects_html()
    seasons, genres = [], []
    sm = re.search(r'"seasons"\s*:\s*(\{.*?\})\s*,\s*"genres"', html or '', re.DOTALL)
    if sm:
        try:
            ...
            pass
    gm = re.search(r'"genres"\s*:\s*(\[.*?\])', html or '', re.DOTALL)
    if gm:
        try:
            genres = [g for g in json.loads(gm.group(1)) if g]
        except Exception:  # noqa
            pass
...
def project_episodes(slug):
    """Egy projekt oldaláról az epizódok: {'title', 'plot', 'episodes':[{id,title,thumb}]}."""
    ...
    title = _txt(tm.group(1)) if tm else slug
    pm = re.search(r'<div class="project-description">(.*?)<div class="project-description-links"',
                   html, re.DOTALL)
    plot = ''
    if pm:
        seg = re.sub(r'<div class="project-cover">.*?</div>\s*</div>', '', pm.group(1),
                     flags=re.DOTALL)
        plot = _txt(seg)
    episodes = []
    seen = set()
    # Az egyes epizódok blokkjai: a "episode-item" nyitóelemek szerint darabolva
    # (a következő episode-item-ig terjed, így nem kell a záró <div>-eket számolni).
    for block in re.split(r'<div class="episode-item">', html)[1:]:
        wm = re.search(r'/episode/watch/id/(\d+)', block)
        if not wm or wm.group(1) in seen:
            continue
        eid = wm.group(1)
        seen.add(eid)
        etm = re.search(r'<div class="episode-title">([^<]+)</div>', block)
        et = _txt(etm.group(1)) if etm else ('rész %s' % eid)
        thm = re.search(r'<img\s+src="([^"]+)"', block)
        thumb = _abs(thm.group(1)) if thm else None
        episodes.append({'id': eid, 'title': et, 'thumb': thumb})
    # tartalék: ha a darabolás nem talált, lazább keresés
    if not episodes:
        for eid in re.findall(r'/episode/watch/id/(\d+)', html):
            if eid in seen:
                continue
            seen.add(eid)
            episodes.append({'id': eid, 'title': 'rész %s' % eid, 'thumb': None})
